...
// -----------------------------
  ...
  _screenToPlanePoint(clientX, clientY, planeY = 0) {
  ...
  _getCopingCandidateHandlePoints() {
    ...
    if (copingMap && Object.keys(copingMap).length) {
      const defs = [
        ["north", "z+"],
        ["south", "z-"],
        ["east", "x+"],
      ...
      defs.forEach(([name, axis]) => {
        const mesh = copingMap[name];
        if (!mesh) return;
        const box = new THREE.Box3().setFromObject(mesh);
        if (box.isEmpty()) return;
        const center = box.getCenter(new THREE.Vector3());
        const point = center.clone();
        if (axis === "x+") point.x = box.max.x + out;
        if (axis === "x-") point.x = box.min.x - out;
        if (axis === "z+") point.z = box.max.z + out;
        if (axis === "z-") point.z = box.min.z - out;
    ...
    const center = box.getCenter(new THREE.Vector3());
    return [
      new THREE.Vector3(center.x, box.max.y + lift, box.min.z - out),
      new THREE.Vector3(center.x, box.max.y + lift, box.max.z + out),
      new THREE.Vector3(box.min.x - out, box.max.y + lift, center.z),
      new THREE.Vector3(box.max.x + out, box.max.y + lift, center.z)
    ];
  }

  _getDimensionHandleWorldTargets() {
    const candidates = this._getCopingCandidateHandlePoints();
    if (!candidates || candidates.length < 4) return null;

    const projected = candidates
      .map((point) => ({ point, screen: this._projectWorldToScreen(point) }))
      .filter((entry) => !!entry.screen);

    if (projected.length < 4) return null;

    const top = projected.reduce((a, b) => (a.screen.y < b.screen.y ? a : b));
    const bottom = projected.reduce((a, b) => (a.screen.y > b.screen.y ? a : b));
    const left = projected.reduce((a, b) => (a.screen.x < b.screen.x ? a : b));
    const right = projected.reduce((a, b) => (a.screen.x > b.screen.x ? a : b));
  ...
  _onDimensionHandlePointerDown(event) {
    ...
    const planeY = Number(handle?.position?.y) || 0;
    const point = this._screenToPlanePoint(event.clientX, event.clientY, planeY);
    if (!point) return;
  ...
  _onDimensionHandlePointerMove(event) {
    ...
    const point = this._screenToPlanePoint(event.clientX, event.clientY, drag.planeY || 0);
    if (!point) return;
