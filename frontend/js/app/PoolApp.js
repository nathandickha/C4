...
export class PoolApp {
  ...
  // -----------------------------
  // Dimension drag handles (shed-style push/pull)
  // -----------------------------
  ...
  setupDimensionHandles() {
    if (this.dimensionHandles?.container) return;
    ...
    const makeHandle = (axis, label) => {
      ...
      el.style.cursor = (axis === "x+" || axis === "x-") ? "ew-resize" : "ns-resize";
      el.textContent = (axis === "x+" || axis === "x-") ? "↔" : "↕";

      const setHover = (hover) => {
        if (this.dimensionHandles?.drag?.axis === axis) return;
        el.style.background = hover ? "rgba(255,255,255,1)" : "rgba(255,255,255,0.96)";
        el.style.boxShadow = hover
          ? "0 5px 16px rgba(0,0,0,0.24)"
          : "0 3px 10px rgba(0,0,0,0.18)";
        el.style.transform = hover ? "translate(-50%, -50%) scale(1.05)" : "translate(-50%, -50%)";
      };

      el.addEventListener("mouseenter", () => setHover(true));
      el.addEventListener("mouseleave", () => setHover(false));
      el.addEventListener("pointerdown", (event) => this._onDimensionHandlePointerDown(event, axis));
    ...
    const items = {
      "x+": makeHandle("x+", "Adjust pool length"),
      "x-": makeHandle("x-", "Adjust pool length"),
      "z+": makeHandle("z+", "Adjust pool width"),
      "z-": makeHandle("z-", "Adjust pool width")
    };
  ...
  destroyDimensionHandles() {
    ...
    if (this.dimensionHandles?.container?.parentNode) {
      this.dimensionHandles.container.parentNode.removeChild(this.dimensionHandles.container);
    }
    this.dimensionHandles = {
      container: null,
      items: {},
  ...
  _setDimensionHandleVisibility(visible) {
    const items = this.dimensionHandles?.items || {};
  ...
  _getPoolHandleWorldPosition(axis) {
    if (!this.poolGroup) return null;

    const center = new THREE.Vector3();
    this.poolGroup.getWorldPosition(center);

    const halfLength = Math.max(0.75, Number(this.poolParams?.length || 0) * 0.5);
    const halfWidth = Math.max(0.75, Number(this.poolParams?.width || 0) * 0.5);

    const bounds = new THREE.Box3().setFromObject(this.poolGroup);
    const point = center.clone();
    point.y = bounds && !bounds.isEmpty() ? bounds.max.y + 0.01 : center.y;
  ...
  _projectWorldToScreen(point) {
    ...
    if (!Number.isFinite(projected.x) || !Number.isFinite(projected.y) || projected.z > 1.05) return null;

    const rect = this.renderer.domElement.getBoundingClientRect();
    return {
      x: rect.left + (projected.x * 0.5 + 0.5) * rect.width,
      y: rect.top + (-projected.y * 0.5 + 0.5) * rect.height,
      visible: projected.z >= -1.1 && projected.z <= 1.05
    };
  }

  _screenToGroundPoint(clientX, clientY) {
  ...
  _onDimensionHandlePointerDown(event, axis) {
    if (!this.poolGroup || !this.dimensionHandles?.items?.[axis]) return;
    if (this.poolParams.shape === "freeform") return;

    event.preventDefault();
    event.stopPropagation();

    const point = this._screenToGroundPoint(event.clientX, event.clientY);
    if (!point) return;

    const center = new THREE.Vector3();
    this.poolGroup.getWorldPosition(center);

    this.captureUndoState(`Drag ${axis === "x+" || axis === "x-" ? "length" : "width"} handle`);
    ...
    const active = this.dimensionHandles.items[axis];
    active.style.background = "rgba(255,255,255,1)";
    active.style.boxShadow = "0 6px 18px rgba(0,0,0,0.28)";
    active.style.transform = "translate(-50%, -50%) scale(1.08)";

    try { active.setPointerCapture?.(event.pointerId); } catch (_) {}
  ...
  _onDimensionHandlePointerMove(event) {
    ...
    if (axis === "x+" || axis === "x-") {
      ...
      const signedDelta = axis === "x+" ? dx : -dx;
      const nextLength = Math.max(minSize, drag.startLength + signedDelta * 2);
      ...
      const signedDelta = axis === "z+" ? dz : -dz;
      const nextWidth = Math.max(minSize, drag.startWidth + signedDelta * 2);
  ...
  _onDimensionHandlePointerUp() {
    ...
    const active = this.dimensionHandles?.items?.[drag.axis];
    if (active) {
      active.style.background = "rgba(255,255,255,0.96)";
      active.style.boxShadow = "0 3px 10px rgba(0,0,0,0.18)";
  ...
  _getPoolScreenBounds() {
    ...
    if (visibleCount < 2 || !Number.isFinite(minX) || !Number.isFinite(maxX) || !Number.isFinite(minY) || !Number.isFinite(maxY)) {
      return null;
    }
  ...
  _updateDimensionHandles() {
    const container = this.dimensionHandles?.container;
    ...
    const screenBounds = this._getPoolScreenBounds();
    if (!screenBounds) {
      this._setDimensionHandleVisibility(false);
      return;
    }
    ...
    const items = this.dimensionHandles.items || {};
    const handleRadius = 23;
    const pad = 18;

    const positions = {
      "x-": { x: screenBounds.minX - pad, y: (screenBounds.minY + screenBounds.maxY) * 0.5 },
      "x+": { x: screenBounds.maxX + pad, y: (screenBounds.minY + screenBounds.maxY) * 0.5 },
      "z+": { x: (screenBounds.minX + screenBounds.maxX) * 0.5, y: screenBounds.minY - pad },
      "z-": { x: (screenBounds.minX + screenBounds.maxX) * 0.5, y: screenBounds.maxY + pad }
    };

    const fitsInView = (pos) => {
      const minX = screenBounds.rect.left + handleRadius;
      const maxX = screenBounds.rect.right - handleRadius;
      const minY = screenBounds.rect.top + handleRadius;
      const maxY = screenBounds.rect.bottom - handleRadius;
      return pos.x >= minX && pos.x <= maxX && pos.y >= minY && pos.y <= maxY;
    };

    Object.entries(items).forEach(([axis, el]) => {
      const pos = positions[axis];
      if (!el || !pos || !fitsInView(pos)) {
