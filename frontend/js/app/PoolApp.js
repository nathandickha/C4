...
// -----------------------------
  // Dimension drag handles (3D billboards, freeform-style)
  // -----------------------------
  ...
  _makeDimensionHandleSprite(axis) {
    const size = 128;
    ...
    ctx.fillText((axis === "x+" || axis === "x-") ? "↔" : "↕", size * 0.5, size * 0.52);
  ...
  setupDimensionHandles() {
    ...
    const meshes = {
      "x+": this._makeDimensionHandleSprite("x+"),
      "x-": this._makeDimensionHandleSprite("x-"),
      "z+": this._makeDimensionHandleSprite("z+"),
      "z-": this._makeDimensionHandleSprite("z-")
    };
  ...
  _intersectDimensionPlane(event, planeY = 0) {
    if (!this.camera || !this.renderer) return null;
    const ndc = this._pointerToNDC(event);
    const raycaster = this.dimensionHandles?.raycaster || new THREE.Raycaster();
  ...
  _getPoolHandleWorldPosition(axis) {
    ...
    const copingMap = this.poolGroup?.userData?.copingSegments || null;
    const sideMap = {
      "x-": "west",
      "x+": "east",
      "z+": "north",
    ...
    const copingKey = sideMap[axis];
    const copingMesh = copingMap?.[copingKey];

    if (copingMesh) {
      const box = new THREE.Box3().setFromObject(copingMesh);
      if (!box.isEmpty()) {
        const center = box.getCenter(new THREE.Vector3());
        const point = center.clone();
        const lift = 0.02;

        if (axis === "x-") point.x = box.min.x;
        else if (axis === "x+") point.x = box.max.x;
        else if (axis === "z+") point.z = box.max.z;
        else if (axis === "z-") point.z = box.min.z;

        point.y = box.max.y + lift;
        return point;
      }
    }
  ...
  _onDimensionHandlePointerDown(event) {
    ...
    const anchor = this._getPoolHandleWorldPosition(axis);
    const point = this._intersectDimensionPlane(event, anchor?.y || 0);
    if (!point) return;
  ...
  _onDimensionHandlePointerMove(event) {
    ...
    const point = this._intersectDimensionPlane(event, drag.planeY);
    if (!point) return;

    const minSize = 2.0;
    if (drag.axis === "x+" || drag.axis === "x-") {
      const dx = point.x - drag.startPoint.x;
      const signedDelta = drag.axis === "x+" ? dx : -dx;
      ...
      const signedDelta = drag.axis === "z+" ? dz : -dz;
  ...
  _updateDimensionHandles() {
    ...
    Object.entries(meshes).forEach(([axis, mesh]) => {
      ...
      const projected = this._projectWorldToScreen(worldPos);
      if (!projected) {
        mesh.visible = false;
        return;
      }
      ...
      const fits =
        projected.x >= projected.rect.left + viewMargin &&
        projected.x <= projected.rect.right - viewMargin &&
        projected.y >= projected.rect.top + viewMargin &&
        projected.y <= projected.rect.bottom - viewMargin;
