...
// -----------------------------
  // Dimension drag handles (DOM overlay anchored to projected coping top face)
  // -----------------------------
  ...
  setupDimensionHandles() {
    ...
    const makeHandle = (key, arrow) => {
      ...
      el.style.cursor = (key === "left" || key === "right") ? "ew-resize" : "ns-resize";
      el.textContent = arrow;
      el.addEventListener("pointerdown", (event) => this._onDimensionHandlePointerDown(event, key));
    ...
    const items = {
      top: makeHandle("top", "↕"),
      bottom: makeHandle("bottom", "↕"),
      left: makeHandle("left", "↔"),
      right: makeHandle("right", "↔")
    };

    document.body.appendChild(container);
    this.dimensionHandles = { container, items, drag: null };
  ...
  destroyDimensionHandles() {
    ...
    this.dimensionHandles = { container: null, items: {}, drag: null };
  }
  ...
  _getProjectedCopingHandlePositions() {
    ...
    const edgeMidpoints = [
      { x: (pts[0].x + pts[1].x) * 0.5, y: (pts[0].y + pts[1].y) * 0.5 },
      { x: (pts[1].x + pts[2].x) * 0.5, y: (pts[1].y + pts[2].y) * 0.5 },
      { x: (pts[2].x + pts[3].x) * 0.5, y: (pts[2].y + pts[3].y) * 0.5 },
      { x: (pts[3].x + pts[0].x) * 0.5, y: (pts[3].y + pts[0].y) * 0.5 }
    ];

    const top = edgeMidpoints.reduce((a, b) => (a.y < b.y ? a : b));
    const bottom = edgeMidpoints.reduce((a, b) => (a.y > b.y ? a : b));
    const left = edgeMidpoints.reduce((a, b) => (a.x < b.x ? a : b));
    const right = edgeMidpoints.reduce((a, b) => (a.x > b.x ? a : b));

    return { top, bottom, left, right, rect };
  }

  _onDimensionHandlePointerDown(event, key) {
    ...
    this.captureUndoState(`Drag ${affectsLength ? "length" : "width"} handle`);
  ...
  _onDimensionHandlePointerMove(event) {
    ...
    if (drag.key === "left" || drag.key === "right") {
      ...
      const signedDelta = drag.key === "top" ? -dz : dz;
  ...
  _onDimensionHandlePointerUp() {
    if (!this.dimensionHandles?.drag) return;
  ...
  _updateDimensionHandles() {
    ...
    const positions = this._getProjectedCopingHandlePositions();
    if (!positions) {
      this._setDimensionHandleVisibility(false);
      return;
    }
    ...
    for (const key of ["top", "bottom", "left", "right"]) {
      ...
      const fits =
        pos.x >= positions.rect.left + margin &&
        pos.x <= positions.rect.right - margin &&
        pos.y >= positions.rect.top + margin &&
        pos.y <= positions.rect.bottom - margin;
