...
// -----------------------------
  ...
  _getDimensionHandleWorldTargets() {
    ...
    const candidates = [
      { key: "north", point: new THREE.Vector3(center.x, box.max.y + lift, box.min.z - out) },
      { key: "south", point: new THREE.Vector3(center.x, box.max.y + lift, box.max.z + out) },
      { key: "west",  point: new THREE.Vector3(box.min.x - out, box.max.y + lift, center.z) },
      { key: "east",  point: new THREE.Vector3(box.max.x + out, box.max.y + lift, center.z) }
    ].map((entry) => ({ ...entry, screen: this._projectWorldToScreen(entry.point) }))
     .filter((entry) => !!entry.screen);

    if (candidates.length < 4) return null;

    const top = candidates.reduce((a, b) => (a.screen.y < b.screen.y ? a : b));
    const bottom = candidates.reduce((a, b) => (a.screen.y > b.screen.y ? a : b));
    const left = candidates.reduce((a, b) => (a.screen.x < b.screen.x ? a : b));
    const right = candidates.reduce((a, b) => (a.screen.x > b.screen.x ? a : b));
