...
export class PoolApp {
  ...
  _getPoolHandleWorldPosition(axis) {
    ...
    if (axis === "x+") point.x += halfLength;
    else if (axis === "x-") point.x -= halfLength;
    else if (axis === "z+") point.z += halfWidth;
    else if (axis === "z-") point.z -= halfWidth;
  ...
  _onDimensionHandlePointerMove(event) {
    ...
    if (axis === "x+" || axis === "x-") {
      const nextLength = Math.max(minSize, Math.abs(point.x - drag.center.x) * 2);
      if (Math.abs(nextLength - this.poolParams.length) > 1e-4) {
        this.poolParams.length = nextLength;
        this._markPoolParamDirty("length");
      }
    } else {
      const nextWidth = Math.max(minSize, Math.abs(point.z - drag.center.z) * 2);
  ...
  _updateDimensionHandles() {
    ...
    Object.entries(items).forEach(([axis, el]) => {
      const worldPos = this._getPoolHandleWorldPosition(axis);
      const screenPos = this._projectWorldToScreen(worldPos);
      if (!el || !screenPos || !screenPos.visible) {
        if (el) el.style.display = "none";
        return;
      }

      const pad = 34;
      const x = Math.min(window.innerWidth - pad, Math.max(pad, screenPos.x));
      const y = Math.min(window.innerHeight - pad, Math.max(pad, screenPos.y));
